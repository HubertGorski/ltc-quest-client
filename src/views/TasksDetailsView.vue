  ...
  const checkHeight = () => {
    ...
    if (mobileLayout) {
      ...
      if (layoutHeight > 179) {
        ...
        showPointsText.value = false;
      } else {
        pointsTemplateHeight.value = 80;
        showPointsText.value = true;
      }
    }
  };
...
<template>
  <div class="mx-4 my-2">
    ...
    <div class="d-flex mobile-layout align-items-stretch">
      ...
      <div class="pointsTemplate__container" @click="handlePointsClick">
        <v-sheet class="pointsTemplate__top ml-4" :style="{ height: pointsTemplateHeight + 'px' }">
          {{ taskDetails.points }} <span v-if="showPointsText" style="font-size: 24px;">pkt</span>
          <v-tooltip activator="parent" location="bottom">Liczba punktów do zdobycia.</v-tooltip>
        </v-sheet>
        <div class="pointsTemplate__bottom ml-4"></div>
      </div>
    </div>
    <v-dialog v-model="showDialog" width="300">
      <v-card>
        <v-card-text class="px-3">
          Liczba punktów do zdobycia: <br> <b style="display: block; text-align: center;">{{ taskDetails.points }}
            pkt</b>
        </v-card-text>
